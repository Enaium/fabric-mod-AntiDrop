        ...
        for (Action action : Action.values()) {
            dispatcher.register(ROOT.then(CommandManager.literal(action.name()).then(CommandManager.argument("item", ItemStackArgumentType.itemStack()).executes(context -> {
                ...
                if (action == Action.ADD) {
                    ...
                    context.getSource().sendFeedback(new TranslatableText("command.action.add", new LiteralText(itemName).styled(style -> style.setColor(Formatting.AQUA).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableText(item.getTranslationKey()))))), false);
                } else if (action == Action.REMOVE) {
                    Config.getModel().item.remove(itemName);
                    context.getSource().sendFeedback(new TranslatableText("command.action.remove", new LiteralText(itemName).styled(style -> style.setColor(Formatting.AQUA).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableText(item.getTranslationKey()))))), false);
                }
