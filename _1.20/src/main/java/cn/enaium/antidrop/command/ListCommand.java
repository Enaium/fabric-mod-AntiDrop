    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(ROOT.then(CommandManager.literal("list").executes(context -> {
            ...
            for (String item : Config.getModel().item) {
                final MutableText itemText = Text.literal(item).styled(style -> style.withColor(Formatting.AQUA).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new HoverEvent.ItemStackContent(Registries.ITEM.get(new Identifier(item)).getDefaultStack()))));
                if (previous == null) {
                    previous = itemText;
                } else {
