...
public class ScreenMouseClickCallbackImpl implements ScreenMouseClickCallback {
    ...
    public ActionResult interact(@Nullable Slot slot, ItemStack cursorStack, SlotActionType slotActionType) {
        if (slot != null && Config.getModel().item.contains(Registries.ITEM.getId(slot.getStack().getItem()).toString()) && slotActionType == SlotActionType.THROW) {
            return ActionResult.FAIL;
        }

        if (slot == null && Config.getModel().item.contains(Registries.ITEM.getId(cursorStack.getItem()).toString()) && slotActionType == SlotActionType.PICKUP) {
            return ActionResult.FAIL;
        }
